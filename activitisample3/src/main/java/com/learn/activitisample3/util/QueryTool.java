...
public class QueryTool {
    ...
    * */
    ...
    public List query(int page, int size, Supplier<? extends Query> queryBy){
        int start = countLimitNum(page, size, startNum);
        int end = countLimitNum(page, size, endNum);
        return queryBy.get().listPage(start,end);
    }
    ...
    public final Supplier<TaskQuery> QUERY_ALL_ACTIVITY_TASK =
            () -> taskQuery.active();

    public final Function<String,TaskQuery> QUERY_TASK_BY_PROCESS_INSTANCE_ID =
            processInstanceId -> taskQuery.processInstanceId(processInstanceId);
}
